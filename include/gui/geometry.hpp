...
inline std::vector<Rect> arrange(Rect area,Axis axis,std::span<const Allocation> items,double gap=0) {
    if(!valid_rect(area)||!std::isfinite(gap)||gap<0||gap>coordinate_limit)
        throw std::invalid_argument("Invalid layout");
    double fixed=0,weight=0;
    for(auto item:items) {
        ...
        fixed+=item.fixed;if(item.fixed==0)weight+=item.weight;
    }
    ...
    for(auto item:items) {
        double length=std::min(std::max(0.0,extent-cursor),
            item.fixed>0?item.fixed:(weight>0?flexible*item.weight/weight:0));
        result.push_back(axis==Axis::horizontal?Rect{area.x+cursor,area.y,length,area.height}:
            Rect{area.x,area.y+cursor,area.width,length});
